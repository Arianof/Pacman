...
class Ghost:
    ...
    def move_up(self, game_map):
        if game_map[self.location[0] - 1][self.location[1]] == 'X':
            return
        else:
            self.location[0] -= 1

    def move_right(self, game_map):
        if game_map[self.location[0]][self.location[1] + 1] == 'X':
            return
        else:
            self.location[1] += 1
    ...
    def possible_moves(self, game_map):
        res = []
        if game_map[self.location[0]][self.location[1] - 1] == ' ':
            res.append([self.location[0], self.location[1] - 1])
        if game_map[self.location[0] - 1][self.location[1]] == ' ':
